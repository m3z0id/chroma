...
void modifyRGB(uint8_t *r, uint8_t *g, uint8_t *b, const std::array<Modifier, 3> &mods) {
    ...
    for (const auto &mod : mods) {
        ...
        std::pair<uint8_t *, uint8_t> &modifierChannel =
                mod.modifierChannel != 0 ? channels.at(getRGBChannelIndex(std::abs(mod.editChannel))) : invalid;
        if ((modifierNegative = mod.editChannel < 0))
            modifierChannel.second = -modifierChannel.second;

        uint8_t *currentChannel = editChannel.first;
        switch (mod.operation) {
        case Operation::ADD:
            if (modifierChannel != invalid)
                *currentChannel += (uint8_t)std::clamp((float)editChannel.second, 0.0f, (float)(255 - modifierChannel.second));
            else
                *currentChannel += (uint8_t)std::clamp(mod.difference, 0.0f, (float)(255 - editChannel.second));
            break;
        case Operation::INVERT:
            if (modifierChannel != invalid)
                *currentChannel = ~modifierChannel.second;
            else
                *currentChannel = ~(uint8_t)mod.difference;
            break;
        case Operation::SET:
            if (modifierChannel != invalid)
                *currentChannel = editChannel.second;
            else
...
void modifyHSL(uint8_t *r, uint8_t *g, uint8_t *b, const std::array<Modifier, 3> &mods) {
    ...
    std::array<std::tuple<float *, float, float>, 3> channels = {{{&h, h, getMaxChannelValue(ColorSpace::HSL, 'h')},
...
void modifyOKLAB(uint8_t *r, uint8_t *g, uint8_t *b, const std::array<Modifier, 3> &mods) {
    ...
    std::array<std::tuple<float *, float, float>, 3> channels = {{{&l, l, getMaxChannelValue(ColorSpace::OKLAB, 'l')},
    ...
    for (const auto &mod : mods) {
        ...
        std::tuple<float *, float, float> &modifierChannel =
                mod.modifierChannel != 0 ? channels.at(getOKLABChannelIndex(std::abs(mod.modifierChannel))) : invalid;
        float *currentChannel = std::get<0>(editChannel);
...
void modifyOKLCh(uint8_t *r, uint8_t *g, uint8_t *b, const std::array<Modifier, 3> &mods) {
    ...
    std::array<std::tuple<float *, float, float>, 3> channels = {{{&l, l, getMaxChannelValue(ColorSpace::OKLCH, 'l')},
    ...
    for (const auto &mod : mods) {
        ...
        std::tuple<float *, float, float> &modifierChannel =
                mod.modifierChannel != 0 ? channels.at(getOKLChChannelIndex(std::abs(mod.modifierChannel))) : invalid;
        float *currentChannel = std::get<0>(editChannel);
