...
inline std::tuple<uint8_t, uint8_t, uint8_t> getRGBFromHSL(float h, float s, float l) {
    ...
    auto toByte = [](float val) -> uint8_t {
        return static_cast<uint8_t>(std::round(std::clamp(val * 255.0f, 0.0f, 255.0f)));
    };
...
inline std::tuple<uint8_t, uint8_t, uint8_t> OklabToRGB(float L, float A, float B) {
    ...
    uint8_t r = static_cast<uint8_t>(std::round(std::clamp(toSRGB(rLinear) * 255.0f, 0.0f, 255.0f)));
    uint8_t g = static_cast<uint8_t>(std::round(std::clamp(toSRGB(gLinear) * 255.0f, 0.0f, 255.0f)));
    uint8_t b = static_cast<uint8_t>(std::round(std::clamp(toSRGB(bLinear) * 255.0f, 0.0f, 255.0f)));
