...
constexpr std::array<float, 256> SRGB_TO_RGB_LUT = {
...
inline float sRGBToLinear(const uint8_t channel) {
    return SRGB_TO_RGB_LUT[channel];
}

inline float fcbrt(const float a) {
    auto u = std::bit_cast<uint32_t>(a);
    u = u / 3 + 709921077;

    const auto x = std::bit_cast<float>(u);
...
inline std::tuple<float, float, float> RGBToOKLAB(const uint8_t r, const uint8_t g, const uint8_t b) {
